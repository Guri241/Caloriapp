...
' POST のときの本文。<FROM> <TO> はURLと同様に日付へ置き換わります
'   例) "{""from"":""<FROM>"",""to"":""<TO>""}"   ← 二重引用符は 2つ重ねます
Private Const API_BODY    As String = ""
...
Public Sub TestApi()
    ...
    Set recs = ApiRecords(url, ApiBody(dFrom, dTo))
...
ErrHandler:
    ...
    MsgBox "エラー " & errNum & " : " & errDesc & vbCrLf & vbCrLf & _
           API_METHOD & "  " & url & vbCrLf & _
           IIf(Len(API_BODY) > 0, vbCrLf & "本文: " & ApiBody(dFrom, dTo), ""), _
           vbCritical, "API接続テスト"
End Sub
...
Private Function FetchApi(ByVal dFrom As Date, ByVal dTo As Date, ByVal fieldMap As Object, _
    ...
    Do While chunkFrom < dTo
        ...
        Set recs = ApiRecords(ApiUrl(chunkFrom, chunkTo), ApiBody(chunkFrom, chunkTo))
...
Private Function ApiUrl(ByVal dFrom As Date, ByVal dTo As Date) As String
    ApiUrl = FillDates(API_URL, dFrom, dTo)
End Function

' 日付を埋め込んだ本文を作る
Private Function ApiBody(ByVal dFrom As Date, ByVal dTo As Date) As String
    ApiBody = FillDates(API_BODY, dFrom, dTo)
End Function

' <FROM> <TO> を日付に置き換える
Private Function FillDates(ByVal text As String, ByVal dFrom As Date, ByVal dTo As Date) As String
    Dim t As String
    t = Replace(text, "<FROM>", Format$(dFrom, API_DATE_FORMAT))
    t = Replace(t, "<TO>", Format$(dTo, API_DATE_FORMAT))
    FillDates = t
End Function

' APIを呼び、レコードの配列を取り出す
Private Function ApiRecords(ByVal url As String, Optional ByVal body As String = "") As Collection
    Dim root As Variant, node As Variant, c As Collection, i As Long

    Set c = New Collection
    root = JsonParse(HttpText(url, body))
...
Private Function HttpText(ByVal url As String, Optional ByVal body As String = "") As String
    Dim http As Object, hs() As String, i As Long, p As Long
    Dim hasContentType As Boolean
    ...
    If Len(API_HEADERS) > 0 Then
        ...
        For i = LBound(hs) To UBound(hs)
            p = InStr(hs(i), ":")
            If p > 1 Then
                http.setRequestHeader Trim$(Left$(hs(i), p - 1)), Trim$(Mid$(hs(i), p + 1))
                If StrComp(Trim$(Left$(hs(i), p - 1)), "Content-Type", vbTextCompare) = 0 Then
                    hasContentType = True
                End If
            End If
        Next i
    ...
    If Len(body) > 0 Then
        If Not hasContentType Then http.setRequestHeader "Content-Type", "application/json"
        http.send body
    Else
        http.send
    End If
