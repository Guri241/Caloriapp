...
Public Sub TestColumns()
    Dim cn As Object, cols As Object, k As Variant
    Dim msg As String, dFrom As Date, dTo As Date, dummy As Date
    ...
    cols(FLD_DATE) = 1
    cols(FLD_LINE) = 1
    If Len(FLD_SHIFT) > 0 Then cols(FLD_SHIFT) = 1
    For Each k In CollectColumns(fieldMap).Keys
        cols(CStr(k)) = 1
    Next k
    ...
    msg = Format$(dFrom, "yyyy/m/d") & " の1日分を、列ごとに取得した結果:" & vbCrLf & vbCrLf
    For Each k In cols.Keys
        msg = msg & ProbeColumn(cn, CStr(k), dFrom, dTo) & vbCrLf
    Next k

    cn.Close
    MsgBox msg, vbInformation, "列ごとの取得テスト"
    Exit Sub
...
ErrHandler:
    ...
    If Not cn Is Nothing Then If cn.State <> 0 Then cn.Close
    On Error GoTo 0
    MsgBox "エラー " & errNum & " : " & errDesc, vbCritical, "列ごとの取得テスト"
End Sub

' 1列だけ取得してみる
Private Function ProbeColumn(ByVal cn As Object, ByVal colName As String, _
                             ByVal dFrom As Date, ByVal dTo As Date) As String
    Dim rs As Object, sql As String, v As String, n As Long

    sql = "SELECT " & Q(colName) & " FROM " & TABLE_NAME & _
    ...
    Do Until rs.EOF
        n = n + 1
        If n = 1 Then v = NzStr(rs.Fields(0).Value)
        If n >= 200 Then Exit Do
        rs.MoveNext
    Loop
    rs.Close
    ProbeColumn = "○ " & colName & "  (" & n & "件, 例: " & v & ")"
    Exit Function
Failed:
    ProbeColumn = "× " & colName & "  → " & Err.Description
    On Error Resume Next
    If Not rs Is Nothing Then If rs.State <> 0 Then rs.Close
End Function
...
Private Function ExecuteQuery(ByVal cn As Object, ByVal sql As String, _
    ...
    ' クライアント側カーソル : ドライバーのカーソルを使わずADOが一括で受け取る
    If USE_CLIENT_CURSOR And Not USE_PARAMETERS Then
        Set rs = CreateObject("ADODB.Recordset")
        rs.CursorLocation = adUseClient
        rs.Open InlineDates(sql, dFrom, dTo), cn, adOpenStatic, adLockReadOnly
        Set ExecuteQuery = rs
        Exit Function
    End If
